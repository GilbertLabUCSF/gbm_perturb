...
for (name in names(data.list)) {
  ...
  df <- subset(df, pvalue < 0.05 & abs(log_fc) > 0.2)
  filtered.list[[name]] <- df
}
```

Stick these into Enrichr and see what we get:

```{r}
dbs <- c("MSigDB_Hallmark_2020","KEGG_2021_Human",
         "WikiPathway_2021_Human","GO_Biological_Process_2021")
for (cluster.name in names(filtered.list)) {
  df <- filtered.list[[cluster.name]]
  
  df.up <- df[df$log_fc > 0, ]
  goi <- df.up$feature
  goi <- gsub("GRCh38-", "", goi)
  enrichROut <- enrichr(goi, dbs)
  msigDB.terms <- enrichROut$MSigDB_Hallmark_2020
  GO.terms <- enrichROut$GO_Biological_Process_2021
  write.table(msigDB.terms, file = sprintf("/raleighlab/data1/czou/gbm_perturb/gbm_perturb_gbm43_explore_outputs/heterogeneity/gbm_pdx_perturb_prkdc/cell_cycle_regressed/enrichr_following_deseq_outputs/up_msigdb_terms_%s.txt", cluster.name))
  write.table(GO.terms, file = sprintf("/raleighlab/data1/czou/gbm_perturb/gbm_perturb_gbm43_explore_outputs/heterogeneity/gbm_pdx_perturb_prkdc/cell_cycle_regressed/enrichr_following_deseq_outputs/up_go_terms_%s.txt", cluster.name))
  write.table(goi, file = sprintf("/raleighlab/data1/czou/gbm_perturb/gbm_perturb_gbm43_explore_outputs/heterogeneity/gbm_pdx_perturb_prkdc/cell_cycle_regressed/enrichr_following_deseq_outputs/up_goi_%s.txt", cluster.name), quote = FALSE, row.names = FALSE)
  Sys.sleep(3)
  
  df.down <- df[df$log_fc < 0, ]
  goi <- df.down$feature
  goi <- gsub("GRCh38-", "", goi)
  enrichROut <- enrichr(goi, dbs)
  msigDB.terms <- enrichROut$MSigDB_Hallmark_2020
  GO.terms <- enrichROut$GO_Biological_Process_2021
  write.table(msigDB.terms, file = sprintf("/raleighlab/data1/czou/gbm_perturb/gbm_perturb_gbm43_explore_outputs/heterogeneity/gbm_pdx_perturb_prkdc/cell_cycle_regressed/enrichr_following_deseq_outputs/down_msigdb_terms_%s.txt", cluster.name))
  write.table(GO.terms, file = sprintf("/raleighlab/data1/czou/gbm_perturb/gbm_perturb_gbm43_explore_outputs/heterogeneity/gbm_pdx_perturb_prkdc/cell_cycle_regressed/enrichr_following_deseq_outputs/down_go_terms_%s.txt", cluster.name))
  write.table(goi, file = sprintf("/raleighlab/data1/czou/gbm_perturb/gbm_perturb_gbm43_explore_outputs/heterogeneity/gbm_pdx_perturb_prkdc/cell_cycle_regressed/enrichr_following_deseq_outputs/down_goi_%s.txt", cluster.name), quote = FALSE, row.names = FALSE)
  Sys.sleep(3)
}
```

We'll generate Enhanced volcano plots for PRKDC.

```{r}
for (cluster.name in names(data.list)) {
  df.all <- data.list[[cluster.name]]
  df.all$feature <- gsub("GRCh38-", "", df.all$feature)
  
  df <- filtered.list[[cluster.name]]
  df$feature <- gsub("GRCh38-", "", df$feature)
  top10.high = df[order(df$log_fc), ][1:10, ]$feature
  top10.low = df[order(-df$log_fc), ][1:10, ]$feature
  goi <- c(top10.high, top10.low)
  ...
  plot <- EnhancedVolcano(
    ...
    ylim = rev(-log(range(df.all$pvalue, na.rm = TRUE), 10)),
    title = cluster.name,
    subtitle = "p < 0.05 and |log_fc| > 0.2",
    selectLab = goi,
    pCutoff = 0.05,
    FCcutoff = 0.2,
    pointSize = 2.0,
  ...
  ggsave(sprintf("/raleighlab/data1/czou/gbm_perturb/gbm_perturb_gbm43_explore_outputs/heterogeneity/gbm_pdx_perturb_prkdc/cell_cycle_regressed/deseq_volcano_outputs/%s.pdf", cluster.name), plot, width = 10, height = 10, useDingbats = FALSE)
}
```
