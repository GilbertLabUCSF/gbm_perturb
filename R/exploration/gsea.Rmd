...
ggsave(paste(WORKING_DIR, "enrichment_bubble_plot_fgsea.pdf", sep = "/"),
       combined.plot, height = 12, width = 18, device = pdf)
```

## Try running a lot of this stuff with Reactome

```{r}
WORKING_DIR <- "/raleighlab/data1/czou/gbm_perturb/gbm_perturb_gbm43_explore_outputs/gsea/noRTNormalized/reactome"
```

Does this make things more interpretable? First, let's run GSEA with Reactome. To do so, we'll have to convert all of our ensemble genes to Entrez gene IDs.
...
for (name in names(data.list)) {
  ...
  deseq_table$entrezid <- unlist(mapping)
  deseq_table <- subset(deseq_table, !is.na(deseq_table$entrezid))
  # Note that this step separates out the tables to different genes
  deseq_table <- deseq_table[!is.na(deseq_table$log_fc), ]
  
  # Group by unique Entrez ID and average
  deseq_table <- deseq_table %>%
    group_by(entrezid) %>%
    summarize(log_fc = mean(log_fc))
  deseq_table <- deseq_table %>% mutate(rank = rank(log_fc,  ties.method = "random"))
  ...
  # Generate a ranked list
  gene_list <- deseq_table$log_fc
  gene_names <- deseq_table$entrezid
  names(gene_list) <- gene_names
  ranked_gene_set_list[[name]] <- gene_list
}
```

Then, we can use the convenient `reactomePathways` function to get ourselves the pathways that we want.
...
for (name in names(ranked_gene_set_list)) {
  ...
  gene.pathways <- reactomePathways(genes)
  gsea.results <- fgsea(pathways = gene.pathways, stats = ranked_gene_set_list[[name]],
                        maxSize = 500, eps = 0.0) # allow arbitrarily low p-values
  fgsea.output.list[[name]] <- gsea.results
}
saveRDS(fgsea.output.list, paste(WORKING_DIR, "fgsea_entrez_reactome_list.rds", sep = "/"))
```
...
ht = Heatmap(heatmap.df,
        ...
        cluster_column_slices = FALSE,
        cluster_columns = TRUE,
        width = ncol(heatmap.df)*unit(4, "mm"),
        height = nrow(heatmap.df)*unit(4, "mm"),
)
pdf(paste(WORKING_DIR, "enrichment_combined_matrix.pdf", sep = "/"),
    width=30,
    height=40,
    useDingbats=FALSE)
draw(ht, heatmap_legend_side = "top", annotation_legend_side = "top")
...
# Take only those pathways into plotting
combined.df <- subset(combined.df, pathway %in% top.pathways)


# Take only the top 10 pathways by enrichment score per perturbation. If there are overlaps, take the union across all of them
top10.pathways <- combined.df %>%
  group_by(Perturbations) %>%
  arrange(desc(abs(ES))) %>%
  slice_head(n = 10) %>%
  ungroup() %>%
