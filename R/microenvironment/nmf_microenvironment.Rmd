# Running NMF with the Microenvironment Exp
...
```{r}
knitr::purl("/raleighlab/data1/czou/gbm_perturb/gbm_perturb/R/microenvironment/nmf_microenvironment.Rmd", output = "/raleighlab/data1/czou/gbm_perturb/gbm_perturb/R/microenvironment/nmf_microenvironment_combined.R")
```
...
We consider genes differentially expressed if they have a LFC absolute value \> 0.1, an adjusted p-value \< 0.05, and sufficient expression to not have their p-values or adjusted p-values eliminated by DESeq.
...
We use the mean squared error and cophenetic correlation as indications of how strong the NMF modeling is at each rank.
...
Next, we want to apply the NMF output based on our chosen rank to get gene modules, and then module groups.
...
Each of these groups represents a collection of (ideally) related genes. In order to give them some character, we'll stick them into Enrichr.
...
Now finally, we want to visualize these gene modules by way of a log fold change matrix. This log fold change matrix will then be shown as a heatmap.
...
## Running NMF, combined cell types
...
OUTPUT.DIR <- "/raleighlab/data1/czou/gbm_perturb/gbm_perturb_sb28_clean_outputs/nmf/ME/prkdc_excluded_05pval/Combined_pval"
RES.LIST.PATH <- "/raleighlab/data1/czou/gbm_perturb/gbm_perturb_sb28_clean_outputs/nmf/ME/prkdc_excluded_05pval/Combined_pval/combined_res_list_2-40_60.rds"
```

### Generate a LFC matrix
...
# Remove Prkdc terms if needed
# deseq.list <- deseq.list[-grep("Prkdc", names(deseq.list))]
...
### Run NMF on the combined matrix
...
for (i in 1:length(module.groups)) {
  setEnrichrSite("Enrichr")
  dbs <- c("MSigDB_Hallmark_2020","KEGG_2021_Human",
           "WikiPathway_2021_Human","GO_Biological_Process_2021", "Reactome_2022")
  goi = module.groups[[i]]
  enrichROut = enrichr(goi, dbs)
  msigDB_terms = enrichROut$MSigDB_Hallmark_2020$Term
  GO_terms = enrichROut$GO_Biological_Process_2021$Term
  reactome_terms = enrichROut$Reactome_2022$Term
  combined_term = paste(msigDB_terms[1], GO_terms[1], reactome_terms[1], sep = "|")
  combined_term_and_genes = c(combined_term, goi)
  combined_term_and_genes = paste(combined_term_and_genes, collapse = "|")
  enrichr_groups[[combined_term]] = goi
  ...
  ontology_groups[i, "msigdb_term"] = msigDB_terms[1]
  ontology_groups[i, "go_term"] = GO_terms[1]
  ontology_groups[i, "reactome_term"] = reactome_terms[1]
  ontology_groups[i, "n_genes"] = length(goi)
  ontology_groups[i, "msigdb_genes"] = enrichROut$MSigDB_Hallmark_2020$Genes[1]
  ontology_groups[i, "go_term_genes"] = enrichROut$GO_Biological_Process_2021$Genes[1]
  ontology_groups[i, "reactome_term_genes"] = enrichROut$Reactome_2022$Genes[1]
  ontology_groups[i, "genes"] = paste(goi, collapse = "|")
  Sys.sleep(3)
}
write.table(ontology_groups, paste(OUTPUT.DIR, sprintf("ontology_groups_rank%s_MaxModules.txt", RANK), sep = "/"), sep = "\t")
...
```{r}
# Generate a list of all genes for later use
list_names = paste(ontology_groups$msigdb_term, ontology_groups$go_term, ontology_groups$reactome_term, sep = "| ")
list_items <- lapply(ontology_groups$genes, function(x) strsplit(x, split = "\\|")[[1]])
enrichr_groups = setNames(list_items, list_names)
all_genes = unlist(enrichr_groups)
```
