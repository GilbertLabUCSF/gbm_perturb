...
PATH.TO.DESEQ.OUTPUT <- "/raleighlab/data1/czou/gbm_perturb/gbm_perturb_sb28_clean_outputs/deseq/SB28_integrated_20231207_ME/sgRNA_gene_sgNegCtrl3_knockouts"
OUTPUT.DIR <- "/raleighlab/data1/czou/gbm_perturb/gbm_perturb_sb28_clean_outputs/nmf/ME/Combined_pval"
RES.LIST.PATH <- "/raleighlab/data1/czou/gbm_perturb/gbm_perturb_sb28_clean_outputs/nmf/ME/Combined_pval/combined_res_list_2-40_60.rds"

## ------------------------------------------------------------------------------------------------------------------------------------------
deseq.list <- list()
input.dirs <- list.dirs(PATH.TO.DESEQ.OUTPUT)[-1]
...
# Subset to the union of DE genes across all perturbations
...
# Subset to the desired cell types
desired.cell.types <- c(
  "Astrocytes",
  "Macrophages",
  "Microglia",
  "OPCs",
...
data <- subset(data, scMRMA_manual %in% desired.cell.types)


## ------------------------------------------------------------------------------------------------------------------------------------------
# Create an sgRNA_gene_cell metadata object so we can pseudobulk against it
data$cell_type_sgRNA_gene <- paste(data$scMRMA_manual, data$sgRNA_gene, "sgNegCtrl", "3", sep = "_")
...
expr_long$cell_type_sgRNA_gene = metadata[expr_long$cell_barcode, "cell_type_sgRNA_gene"]
avg_data = expr_long %>%
  dplyr::group_by(gene, cell_type_sgRNA_gene) %>%
  dplyr::summarise(mean_expression = mean(expression, na.rm = TRUE))
avg_matrix = tidyr::pivot_wider(avg_data, names_from = cell_type_sgRNA_gene, values_from = mean_expression)
...
deMatsig <- deMatsig[, intersect(perturbs.in.deseq, colnames(avg_matrix))]
...
saveRDS(res.list, sprintf("%s/combined_res_list_2-40_60.rds",
                          OUTPUT.DIR))
